...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const starCount = Math.min(100, Math.floor((window.innerWidth * window.innerHeight) / 12000)); // Medium amount
    
    for (let i = 0; i < starCount; i++) {
      stars.push({
        x: Math.random() * canvas.width,
        y: Math.random() * canvas.height,
        size: Math.random() * 0.8 + 0.4, // Medium size
    ...
    const initPixi = async () => {
      ...
      app.ticker.add(() => {
        ...
        meteorsRef.current.forEach((meteor) => {
          ...
          // Skip rendering while in delay phase (negative progress)
          ...
          // Fade out effect - strong at start, fades in second half
          const fadeStart = 0.5; // Stay strong for first 50%
          const fadeOut = meteor.progress > fadeStart 
            ? 1 - ((meteor.progress - fadeStart) / (1 - fadeStart))
            : 1;
          const fade = Math.pow(fadeOut, 2); // Faster fade once it starts
          ...
          // Draw the main tail - line segments (slimmer)
          for (let i = 1; i < trailLength; i++) {
            const prev = meteor.trail[i - 1];
            const curr = meteor.trail[i];
          ...
          // Draw bright core along last part of trail (slimmer)
  ...
  const createSingleMeteor = useCallback((x: number, y: number, delayProgress: number = 0, angleOffset: number = 0) => {
    ...
    // Base 45-degree angle with offset for variety
    const baseAngle = 45 * (Math.PI / 180);
    const angle = baseAngle + angleOffset;
    
    const distance = 2000 + Math.random() * 400;
    
    // Direction based on position: right side → go left, left side → go right
    ...
    // Control point at midpoint for straight line (slight curve for realism)
  ...
  const spawnMeteor = useCallback((x: number, y: number) => {
    ...
    // Direction for offset calculation
    const screenMidX = window.innerWidth / 2;
    const directionX = x > screenMidX ? -1 : 1;

    // Randomly decide how many meteors: 1 (60%), 2 (30%), 3 (10%)
    const rand = Math.random();
    const meteorCount = rand < 0.6 ? 1 : rand < 0.9 ? 2 : 3;

    // Spawn lead meteor (base 45° angle)
    createSingleMeteor(x, y, 0, 0);

    // Spawn trailing meteors with offset, delay, and slight angle difference
    if (meteorCount >= 2) {
      // 2nd meteor: 47° (~2 degrees more = 0.035 radians)
      const offset2X = -directionX * 80 + (Math.random() - 0.5) * 30;
      const offset2Y = -60 + (Math.random() - 0.5) * 20;
      const angle2 = 0.035; // +2 degrees → 47°
      setTimeout(() => {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (!hoverTimeoutRef.current) {
      ...
      // Then spawn every 500ms while hovering
